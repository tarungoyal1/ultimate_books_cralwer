    ...
    def parse(self, response):
        ...
        try:
            ...
            for a in authors:
                ...
                author['author_link']  = self.clean_url(response.urljoin(a.xpath(".//span[@class='a-declarative']/a/@href").get()), pattern_ref)
                authors_data.append(author)

            book['authors_data'] = authors_data

            book['book_rating_average'] = response.xpath("//div[@id='averageCustomerReviews']//span[contains(@class, 'a-icon-alt')]/text()").get().partition('out')[0].rstrip()
            book['book_rating_count'] = response.xpath("//a[@id='acrCustomerReviewLink']/span/text()").get().partition('rating')[0].rstrip()
            ...
            if isBestseller:
                ...
                for data in isBestseller:
                    book['bestseller_data']["bestseller_link"]      = self.clean_url(response.urljoin(data.xpath(".//a/@href").get()), pattern_ref)
